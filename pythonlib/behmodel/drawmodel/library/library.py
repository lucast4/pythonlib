...
from pybpl.util.general import aeq

# USE_HIST = True
LIB_DATA_PATH = '/data1/code/python/pyBPL/lib_data/'
...
class Library(object):
    ...
    def __init__(self, lib_dir=None, use_hist=False):
        ...
        self.Thetas, self.logpThetas = self.get_default_rotation(nbins=8)

        # Check consistency of the library
        self.check_consistent()


    def get_default_rotation(self, nbins):
        from math import pi

        # 1) get bin angles
        thetas = torch.linspace(0, 2*pi, nbins+1)
        thetas = thetas[1:] - torch.diff(thetas)/2

        # 2) get distribution
        probs = torch.ones_like(thetas)
        log_probs = torch.log(probs/torch.sum(probs))
