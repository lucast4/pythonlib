...
def initial_clustering_extract_and_plot_clusters(DS, WHICH_LEVEL, WHICH_BASIS_SET, which_shapes, WHICH_FEATURE, SDIR,
    ...
    ##### SAVE
    if False: # Now just saving in one location (see below)
        import pickle
        path = f"{SDIR}/DS.pkl"
    ...
    if do_save:
        ...
        assert WHICH_LEVEL=="trial", "assuming trial. otherwise this bad, will overwrite below... Soluytion: add WHICH_LEVEL to dir name"
        DIR = f"{PATH_ANALYSIS_OUTCOMES}/recordings/main/EXPORTED_BEH_DATA/DS/char_strokes_clusters/{animal}/basis={WHICH_BASIS_SET}-shapes={which_shapes}/{date}"
