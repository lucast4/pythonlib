...
def alignmentcompute_wrapper(HACK_SKIP_FAILED_DATE_FOR_NOW, SKIP_IF_DONE = False, DEBUG=False, DEBUG_BREAK=False):
    """
    ...
    SAVEDIR = f"/lemur2/lucas/analyses/recordings/main/syntax_good/alignments"
            ...
            try:
                ...
                for _iter in range(n_iter_splits):
                    ...
                    if DEBUG:
                        ...
                        dfres_dot = pd.DataFrame(RES_DOT)
                        assert False, "pause here, for debugging"
...
def alignment_load_extracted_dot_products(SAVEDIR, across_version, prune_if_too_few_trials, levels_var_required,
        HACK_SKIP_FAILED_DATE_FOR_NOW=False):
    """
    ...
    SAVEDIR_THIS = f"{SAVEDIR_PLOTS}/across_version={across_version}-prune_if_too_few_trials={prune_if_too_few_trials}-levels_var_required={levels_var_required}"
    ...
    from pythonlib.tools.pandastools import aggregGeneral
    print("Before agg over iter: ", len(DFRES_DOT))
...
def alignment_plot_overview_1(DFRES_DOT, DFRES_DOT_AGG, SAVEDIR_THIS):
    ...
    x_order = ["chunk_within_rank", "chunk_rank_global", "across_variables"]
